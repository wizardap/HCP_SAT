...
def run_comparison(graph_path, solver_name, enable_probing=True):
    ...
    # Choose encoder (PbLib if available, else NSC)
    try:
    ...
    for name, init_successor in configs:
        ...
        nof_vars = hcpSolver.context.total_vars()
        nof_clauses = hcpSolver.context.total_clauses()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Compare baseline CRT vs Preprocessed CRT.")
    ...
    print(f"Comparing solvers on {len(graph_files)} instances...")
    ...
    for path in graph_files:
        ...
        try:
            res = run_comparison(path, args.solver, enable_probing=enable_probing)
